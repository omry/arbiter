...
ARBITER_SYSTEMD_DIR="$install_root/etc/systemd/system" ARBITER_FAKE_SYSTEMCTL_LOG="$install_root/systemctl.log" PATH="$install_root/bin:$PATH" sudo ./arbiter-docker install --to "$install_root/opt/arbiter" --user "$current_user" --group "$current_group" --no-start --skip-static-config-check | rewrite_install_output
