...
from arbiter_server import version as version_module
...
def test_distribution_version_prefers_source_pyproject(
    ...
    pyproject = tmp_path / "server" / "pyproject.toml"
    package_file = tmp_path / "server" / "src" / "arbiter_server" / "version.py"
    package_file.parent.mkdir(parents=True)
    pyproject.parent.mkdir(parents=True, exist_ok=True)
    pyproject.write_text(
        """[project]
...
def test_distribution_version_does_not_escape_venv_for_source_pyproject(
    ...
    checkout_pyproject = tmp_path / "server" / "pyproject.toml"
    package_file = (
        tmp_path
        / ".venv"
        / "lib"
...
def test_arbiter_server_version_reads_server_distribution(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fake_distribution_version(
        ...
        *,
        ...
        assert distribution_name == "arbiter-server"
        return "1.2.3"
    ...
    assert version_module.arbiter_server_version() == "1.2.3"
