...
import (
	...
	"github.com/omry/arbiter/client-go/internal/cli"
	"github.com/omry/arbiter/client-go/internal/testutil"
)
...
func TestVersionMatchesArbiterCorePackage(t *testing.T) {
	coreVersion := corePyprojectVersion(t)

	if cli.Version != coreVersion {
		t.Fatalf("Go client version %q does not match arbiter-core version %q; run go generate ./internal/cli", cli.Version, coreVersion)
	}
}
...
func corePyprojectVersion(t *testing.T) string {
	t.Helper()
	path := filepath.Join("..", "..", "..", "core", "pyproject.toml")
